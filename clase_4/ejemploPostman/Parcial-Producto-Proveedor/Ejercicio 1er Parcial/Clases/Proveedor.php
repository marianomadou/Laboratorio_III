    ...
    public static function MostrarTablaFotosBackUp($rutaProveedores, $rutaFotosBackUp)
    {
        ...
        else
        {
            ...
            foreach ($directorios as $key => $dir)
            {
                ...
                {
                    $stdObj = new stdClass();
                    $idAux = explode("_", $dir)[0];
                    $strAux = explode("_", $dir)[1];
                    $proveedor = self::TraerProveedorPorId($rutaProveedores, $idAux);
                    if(!$proveedor)
                    {
                    ...
                    $stdObj->fecha = substr($strAux, 6, 2)."/".substr($strAux, 4, 2)."/".substr($strAux, 0, 4);
